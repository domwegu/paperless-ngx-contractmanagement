...
/**
 * PaperlessClient
 * Wird pro Request mit den Mandanten-Credentials instanziiert.
 * Kein globaler Singleton — jeder Mandant hat ggf. eigene URL + Token.
 */
@Injectable()
export class PaperlessClient {
  ...
  /**
   * Erzeugt einen konfigurierten Axios-Client für einen Mandanten.
   */
  buildClient(baseUrl: string, apiToken: string): AxiosInstance {
    ...
    // Response-Interceptor für einheitliches Fehlerhandling
    client.interceptors.response.use(
      ...
      (err) => {
        ...
        throw new BadGatewayException(`Paperless API nicht erreichbar: ${message}`);
      },
    );
  ...
  async getDocuments(
    http: AxiosInstance,
    params?: { page?: number; page_size?: number; tags__id__all?: string; search?: string },
  ): Promise<PaperlessPaginatedResult<PaperlessDocument>> {
  ...
  async uploadDocument(
    ...
    if (options.tags?.length) {
      options.tags.forEach((t) => form.append('tags', String(t)));
    }

    const res = await http.post('/documents/post_document/', form, {
      headers: form.getHeaders(),
      timeout: 60_000,     // Upload darf länger dauern
    });

    // Paperless gibt task_id zurück — wir pollen kurz auf die doc-id
    const taskId = res.data?.task_id ?? res.data?.id;
    if (!taskId) throw new BadGatewayException('Paperless: Kein Task-ID im Upload-Response');

    return this.pollTaskForDocumentId(http, taskId);
  }

  async updateDocument(
    http: AxiosInstance,
    id: number,
    patch: Partial<{ title: string; tags: number[]; correspondent: number; document_type: number }>,
  ...
  async findOrCreateTag(http: AxiosInstance, name: string): Promise<PaperlessTag> {
    ...
    return existing ?? this.createTag(http, name);
  }
  ...
  async findOrCreateCorrespondent(
    http: AxiosInstance,
    name: string,
    ...
    return existing ?? this.createCorrespondent(http, name);
  }
  ...
  async findOrCreateDocumentType(
    http: AxiosInstance,
    name: string,
  ...
  /**
   * Pollt den Paperless Task-Endpoint bis die Document-ID verfügbar ist.
   * Paperless verarbeitet Uploads asynchron.
   */
  private async pollTaskForDocumentId(
    ...
    for (let i = 0; i < maxAttempts; i++) {
      ...
      try {
        ...
        // Netz-Fehler beim Pollen ignorieren, weiter versuchen
      }
    }
